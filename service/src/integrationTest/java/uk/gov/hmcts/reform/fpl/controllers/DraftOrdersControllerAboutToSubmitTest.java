...
class DraftOrdersControllerAboutToSubmitTest extends AbstractControllerTest {
    ...
    private static final String DRAFT_SEALED_ORDER_FILE_NAME = "draft-standard-directions-order.pdf";
    ...
    void setup() {
        ...
        given(uploadDocumentService.uploadPDF(eq(PDF), any())).willReturn(DOCUMENT);
    }
    ...
    private CaseDetails getCaseDetailsWithSdoStatus(OrderStatus status) {
        ...
        return CaseDetails.builder()
            .data(directionsWithShowHideValuesRemoved()
                ...
                .put("standardDirectionOrder", Order.builder().orderStatus(status).build())
    ...
    private Order expectedOrder(OrderStatus status) {
        String fileName;

        if (status == SEALED) {
            fileName = SEALED_ORDER_FILE_NAME;
        } else {
            fileName = DRAFT_SEALED_ORDER_FILE_NAME;
        }
