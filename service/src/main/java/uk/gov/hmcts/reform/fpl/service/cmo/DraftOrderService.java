...
public class DraftOrderService {
    ...
    public UploadDraftOrdersData getInitialData(CaseData caseData) {
        ...
        return UploadDraftOrdersData.builder()
            ...
            .draftOrderHearings(allHearingsList)
    ...
    public UploadDraftOrdersData getDraftsInfo(CaseData caseData) {
        ...
        UploadDraftOrdersData.UploadDraftOrdersDataBuilder newEventDataBuilder = UploadDraftOrdersData.builder()
            .draftOrderKinds(eventData.getDraftOrderKinds())
            ...
            .draftOrderHearings(regenerateList(eventData.getDraftOrderHearings(), caseData.getHearingDetails(),
                NO_HEARING));

        if (eventData.getDraftOrderKinds().contains(HearingOrderKind.CMO)) {
        ...
        if (eventData.getDraftOrderKinds().contains(HearingOrderKind.C21)) {
            newEventDataBuilder.hearingDraftOrders(
                defaultIfNull(eventData.getHearingDraftOrders(), getC21Drafts(caseData, selectedHearingId)));
        }
    ...
    private List<Element<HearingOrder>> getC21Drafts(CaseData caseData, UUID selectedHearingId) {
        ...
        final List<Element<HearingOrder>> draftOrders = unwrapElements(caseData.getDraftHearingOrdersBundles()).stream()
    ...
    public UUID updateCase(UploadDraftOrdersData eventData, List<Element<HearingBooking>> hearings,
        ...
        final UUID selectedHearingId = eventData.getSelectedHearingId(mapper);
        ...
        if (eventData.getDraftOrderKinds().contains(HearingOrderKind.CMO)) {
            ...
            insertOrder(cmoDrafts, order, previousCmoId);
        ...
        if (eventData.getDraftOrderKinds().contains(HearingOrderKind.C21)) {
            eventData.getHearingDraftOrders().forEach(o -> {
                o.getValue().setDateSent(time.now().toLocalDate());
                o.getValue().setStatus(SEND_TO_JUDGE);
            });
            addOrdersToBundle(bundles, eventData.getHearingDraftOrders(), hearing, C21);
        }
    ...
    public List<Element<HearingOrdersBundle>> migrateCmoDraftToOrdersBundles(CaseData caseData) {
        ...
        List<Element<HearingOrdersBundle>> bundles = defaultIfNull(caseData.getDraftHearingOrdersBundles(),
            new ArrayList<>());
    ...
    private void insertOrder(List<Element<HearingOrder>> unsealedOrders, Element<HearingOrder> order,
                             Optional<UUID> id) {
        if (id.isPresent()) {
