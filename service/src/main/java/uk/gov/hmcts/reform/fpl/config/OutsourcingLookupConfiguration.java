...
@Slf4j
@Configuration
public class OutsourcingLookupConfiguration {

    private final Map<String, List<String>> mapping;

    public OutsourcingLookupConfiguration(@Value("${fpl.outsourcing.mapping:}") String config) {
        if (StringUtils.isBlank(config)) {
            mapping = emptyMap();
            log.warn("Outsourcing mapping is missing or empty");
        } else {
