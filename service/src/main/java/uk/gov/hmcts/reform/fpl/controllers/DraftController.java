...
public class DraftController {
        ...
        if (caseData.getStandardDirectionOrder() == null) {
            ...
            List<Map<String, Object>> directions =
                (List<Map<String, Object>>)
                    ((Map<String, Object>) caseDetails.getData().get("standardDirectionOrder")).get("directions");
    ...
    public AboutToStartOrSubmitCallbackResponse midEvent(
        ...
        DocmosisDocument docmosisDocument = documentGeneratorService
            .generateDocmosisDocument(preparePlaceholders(caseData), DocmosisTemplates.SDO);
    ...
    private Map<String, Object> preparePlaceholders(CaseData caseData) {
        HearingBooking hearingBooking = caseData
            .getHearingDetails() != null && caseData.getHearingDetails().get(0) != null
            ? caseData.getHearingDetails().get(0).getValue() :
        ...
        return Map.of(
            ...
            "hearingVenue", StringUtils.defaultIfEmpty(hearingBooking.getVenue(),
                "DEFAULT"),
            "hearingDate", StringUtils.defaultIfEmpty(hearingBooking.getDate() != null
                ? hearingBooking.getDate().toString() : "DEFAULT", "DEFAULT"),
            "preHearingAttendance", StringUtils.defaultIfEmpty(hearingBooking.getPreHearingAttendance(),
                "DEFAULT"),
            "hearingTime", StringUtils.defaultIfEmpty(hearingBooking.getTime(), "DEFAULT"),
            "complianceDeadline", StringUtils.defaultIfEmpty(caseData.getDateSubmitted().plusWeeks(26).toString(),
                "DEFAULT"), //REFACTOR !!!
    ...
    private List<Map<String, String>> prepareDirections(CaseData caseData) {
        return caseData.getStandardDirectionOrder().getDirections()
            ...
            .map(direction -> Map.of(
                "title",
                direction.getType() + " comply by: " + (direction.getCompleteBy() != null
                    ? direction.getCompleteBy().toString() : " unknown"),
