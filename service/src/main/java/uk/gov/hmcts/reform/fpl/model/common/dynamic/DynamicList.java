...
public class DynamicList {
    ...
    /**
     * Merges the current {@link DynamicList} with the passed {@link DynamicList} returning a new {@link DynamicList}.
     * The selected item is merged based on the label if no matching item is found.
     *
     * @param other List to merge with
     * @return a new instance of a merged list
     */
    public DynamicList merge(DynamicList other) {
        if (other == null) {
            return this;
        }

        List<DynamicListElement> list = other.getListItems();
        ...
        if (list.contains(this.value)) {
            ...
            // Get the item that has a matching label
