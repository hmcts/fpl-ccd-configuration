...
export class QueryManagement extends BasePage {
    ...
    async askFollowupQuery() {
        await this.page.getByRole('button', {name: 'query by LA'}).click();
    ...
    async respondToQuery(closeTheQuery: boolean = false) {
        ...
        await this.respondDetail.fill('Answered to the query raised by LA');
    ...
    async expandQuery(queryName: string) {
        await this.page.getByRole('button', {name: queryName}).click();
    }
}
