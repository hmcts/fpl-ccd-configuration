...
export class CourtServicesNeeded extends BasePage {
    ...
    async CourtServicesSmoketest() {
        await expect(this.courtServicesNeededHeading).toHaveText('Court services needed');
        await this.interpreterOptional.getByLabel('Yes').check();
        await expect(this.giveDetailsIncludingPerson).toHaveText('Give details including person');
