...
export function HearingDetailsMixin(BasePage) {
  return class extends BasePage {
    ...
    async completeHearingDetails() {
      ...
      await this.page.getByRole('textbox', { name: 'Day' }).press('Tab');
      await this.page.getByRole('textbox', { name: 'Year' }).fill('2');
      await this.page.getByRole('textbox', { name: 'Month' }).press('Tab');
      await this.page.getByRole('textbox', { name: 'Year' }).fill('2024');
      await this.page.getByRole('spinbutton', { name: 'Hour' }).fill('01');
      await this.page.getByRole('textbox', { name: 'Year' }).press('Tab');
      await this.page.getByLabel('Set number of hours and').check();
      await this.page.getByLabel('Hearing length, in hours').click();
      await this.page.getByLabel('Hearing length, in hours').fill('1');
      await this.page.getByLabel('Hearing length, in hours').press('Tab');
