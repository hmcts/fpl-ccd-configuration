...
test.describe('Manage LAs / Transfer to court @sessionreuse', () => {
    ...
    test('CTSC transfer to a new court and submit case',
        async ({ page, signInPage, ctscUser,manageLaTransferToCourts }) => {
            ...
            await expect(manageLaTransferToCourts.page.getByText('Family Court sitting at Swansea')).toBeHidden();
            await expect(manageLaTransferToCourts.page.getByText('Family Court sitting at Central Family Court')).toBeVisible();
    ...
    test('CTSC gives access to another local authority',
        async ({ page, signInPage,ctscUser, manageLaTransferToCourts }) => {
           ...
           if(urlConfig.env.toUpperCase() === 'DEMO'){
            ...
            await expect(manageLaTransferToCourts.page.getByText('Applicant 2')).toBeVisible();
            await expect(manageLaTransferToCourts.page.getByText('London Borough Hillingdon')).toBeVisible();
    ...
    test('CTSC removes access',
        async ({ page, signInPage, ctscUser,manageLaTransferToCourts }) => {
            ...
            await manageLaTransferToCourts.tabNavigation('People in the case');
            await expect(manageLaTransferToCourts.page.getByText('Applicant 2')).toBeHidden();
            await expect(manageLaTransferToCourts.page.getByText('London Borough Hillingdon')).toBeHidden();
        })
    test('CTSC tranfers to another local authority',
        async ({ page, signInPage, ctscUser,manageLaTransferToCourts }) => {
            ...
            await manageLaTransferToCourts.tabNavigation('People in the case');
            await expect(manageLaTransferToCourts.page.getByText('Swansea City Council')).toBeHidden();
            await expect(manageLaTransferToCourts.page.getByText('London Borough Hillingdon')).toBeVisible();
            await manageLaTransferToCourts.tabNavigation('Summary');
            await manageLaTransferToCourts.page.reload();
            await expect(manageLaTransferToCourts.page.getByText('Family Court sitting at Swansea')).toBeHidden();
            await expect(manageLaTransferToCourts.page.getByText('Family Court sitting at West London')).toBeVisible();
        })
});
