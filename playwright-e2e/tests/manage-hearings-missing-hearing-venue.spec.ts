...
import caseData from '../caseData/caseWithHearingWithValidHearingVenue.json' assert { type: 'json' };
import caseDataWithMissingHearingVenue from '../caseData/caseWithHearingWithMissingHearingVenue.json' assert { type: 'json' };
...
test.describe('manage hearings', () => {
  ...
  test('CTSC admin add new hearing with missing venue @xbrowser',
  ...
  test('Previous hearing venue is hidden when previous hearing venue id is null',
    async ({page,signInPage,manageHearings}) => {
      caseName = 'CTSC manage hearings ' + dateTime.slice(0, 10);
