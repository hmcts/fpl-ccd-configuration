...
test.describe('manage orders', () => {
    ...
    test('CTSC uploads Recovery of Child (C29)', async ({ page, signInPage, orders }) => {
        ...
        await orders.selectOrder('Recovery of Child (C29)');
        await orders.clickContinue();

        await orders.ctscUploadsRecoveryOfChild();
        await orders.clickContinue();
        ...
        await orders.tabNavigation('Orders');
        await expect(page.getByRole('link', { name: 'c29_recovery_of_a_child.pdf', exact: true })).toBeVisible();
