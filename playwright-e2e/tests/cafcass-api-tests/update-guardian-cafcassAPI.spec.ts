...
import {cafcassUpdateGuardianDetails, GAURDIAN_DETAILS} from "../../utils/cafcass-api-test-helper";
...
test.describe('CafcassAPI Update Guardian Details', () => {
    ...
    test.only(' Cafcass user update the guardian details', async ({request, page, signInPage}) => {
            ...
            let response = await cafcassUpdateGuardianDetails(request, authToken.cafcassAuth, caseNumber, GAURDIAN_DETAILS);
            ...
            await expect(page.getByText('Guardian 2', {exact: true})).toBeVisible();
    ...
    test('Cafcass Update guardian details by user without cafcass role', async ({request}) => {
        ...
        await updateCase('Not authorised user update guardian details' + dateTime.slice(0, 10), caseNumber, submitCase);


        let response = await cafcassUpdateGuardianDetails(request, authToken.systemAuth, caseNumber, GAURDIAN_DETAILS);
        //assertion
