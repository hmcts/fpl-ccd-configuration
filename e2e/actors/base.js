...
module.exports = {
  ...
  async logWithHmctsAccount() {
    const hmctsLoginIn = 'div.win-scroll';
      if (!config.smokeTestUser.email || !config.smokeTestUser.password) {
      ...
      this.fillField('//input[@type="text"]', config.smokeTestUser.email);
      this.fillField('//input[@type="password"]', config.smokeTestUser.password);
      this.wait(0.2);
      this.click('Sign in');
