...
xScenario('HMCTS super user removes a generated order through \'Create or upload an order - legacy\' from a case @nightlyOnly', async ({I, caseViewPage, removalToolEventPage}) => {
...
xScenario('HMCTS super user removes a generated order through \'Manage orders\' from a case @nightlyOnly', async ({I, caseViewPage, removalToolEventPage}) => {
  await setupScenario(I);
...
xScenario('HMCTS super user removes a sealed cmo from a case @nightlyOnly', async ({I, caseViewPage, removalToolEventPage}) => {
  await setupScenario(I);
...
xScenario('HMCTS super user removes a sdo from a case @nightlyOnly', async ({I, caseViewPage, removalToolEventPage}) => {
  await setupScenario(I);
...
xScenario('HMCTS super user removes a draft cmo from a case @nightlyOnly', async ({I, caseViewPage, removalToolEventPage}) => {
  await setupScenario(I);
  ...
  const labelToSelect = 'Draft case management order sent on '
    ...
    + ', ' + orderToRemove.order.document_filename;
...
xScenario('HMCTS super user removes an agreed cmo from a case @nightlyOnly', async ({I, caseViewPage, removalToolEventPage}) => {
  await setupScenario(I);
...
xScenario('HMCTS super user removes a draft order from a case @nightlyOnly', async ({I, caseViewPage, removalToolEventPage}) => {
  await setupScenario(I);
  ...
  const labelToSelect = 'Draft order sent on '
    + moment(orderToRemove.dateSent).format('D MMMM YYYY')
    + ' for ' + orderToRemove.title + ', ' + orderToRemove.order.document_filename;

  await removeOrder(I, caseViewPage, removalToolEventPage, labelToSelect, false);
  caseViewPage.checkTabIsNotPresent(caseViewPage.tabs.draftOrders);
});

xScenario('HMCTS super user removes an additional application from the case @nightlyOnly', async ({I, caseViewPage, removalToolEventPage}) => {
  ...
  removalToolEventPage.selectAdditionalApplicationToRemove(labelToSelect);
