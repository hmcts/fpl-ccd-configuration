...
xScenario('Solicitor cannot request representation before case submission and Cafcass solicitor is set @nightlyOnly', async ({I, caseListPage, caseViewPage, submitApplicationEventPage, noticeOfChangePage}) => {
...
xScenario('HMCTS Confirm that a main solicitor in not assigned for all the children yet @nightlyOnly', async ({I, caseViewPage, enterChildrenEventPage, noticeOfChangePage}) => {
  await setupScenario(I);
...
xScenario('HMCTS assign a main solicitor for all the children @nightlyOnly', async ({I, caseViewPage, enterChildrenEventPage}) => {
  await setupScenario(I);
  await I.navigateToCaseDetailsAs(config.hmctsAdminUser, caseId);
  await caseViewPage.goToNewActions(config.administrationActions.amendChildren);
  await I.goToNextPage();
...
xScenario('HMCTS assign a different solicitor for some of the children @nightlyOnly', async ({I, caseViewPage, enterChildrenEventPage}) => {
  await setupScenario(I);
  await I.navigateToCaseDetailsAs(config.hmctsAdminUser, caseId);
  await caseViewPage.goToNewActions(config.administrationActions.amendChildren);
  await I.goToNextPage();
...
xScenario('Solicitor can request representation via NOC only after case submission and Cafcass solicitor is set @nightlyOnly', async ({I, caseListPage, noticeOfChangePage, caseViewPage}) => {
  await setupScenario(I);
