...
Scenario('Bulk scan user uploads Scanned Documents', async (I, caseViewPage, handleSupplementaryEvidenceEventPage, attachScannedDocsEventPage) => {
  await I.signIn(config.bulkScanEmail, config.bulkScanPassword);
...
Scenario('HMCTS admin can see Documents scanned in with Bulk Scan', async (I, caseViewPage) => {
  ...
  I.seeAnswerInTab(1, 'Scanned Documents 1', 'Document type', 'Form');
  ...
  I.seeAnswerInTab(8, 'Scanned Documents 1', 'Exception record reference', 'Example exception record');
});
