...
async function setupScenario(I) {
  if (!caseIdAndName) {
    caseIdAndName = await I.submitNewCaseWithData(mandatoryWithMultipleChildren);
    await api.grantCaseAccess(caseIdAndName.caseId, config.hillingdonLocalAuthorityUserOne, '[SOLICITOR]');
    await api.grantCaseAccess(caseIdAndName.caseId, config.privateSolicitorOne, '[SOLICITORA]');
  }
  await I.navigateToCaseDetailsAs(config.hmctsAdminUser, caseIdAndName.caseId);
}
...
Scenario('HMCTS Admin and LA upload confidential and non confidential further evidence documents', async ({I, caseViewPage, manageDocumentsEventPage, manageDocumentsLAEventPage}) => {
  ...
  await I.navigateToCaseDetailsAs(config.newSwanseaLocalAuthorityUserOne, caseIdAndName);
...
xScenario('HMCTS Admin and LA upload confidential and non confidential respondent statement @nightlyOnly', async ({I, caseViewPage, manageDocumentsEventPage, manageDocumentsLAEventPage}) => {
  ...
  await I.navigateToCaseDetailsAs(config.swanseaLocalAuthorityUserOne, caseIdAndName);
  await caseViewPage.goToNewActions(config.applicationActions.manageDocumentsLA);
  ...
  await I.navigateToCaseDetailsAs(config.hillingdonLocalAuthorityUserOne, caseIdAndName);
  caseViewPage.selectTab(caseViewPage.tabs.furtherEvidence);
...
xScenario('HMCTS Admin and LA upload confidential C2 supporting documents @nightlyOnly', async ({I, caseViewPage, manageDocumentsEventPage, manageDocumentsLAEventPage, uploadAdditionalApplicationsEventPage}) => {
  ...
  await api.pollLastEvent(caseIdAndName, config.internalActions.updateCase);
...
xScenario('Solicitor with access uploads documents @nightlyOnly', async ({I, manageDocumentsEventPage, caseViewPage}) => {
  await setupScenario(I);
  await I.navigateToCaseDetailsAs(solicitor, caseIdAndName);
  await caseViewPage.goToNewActions(config.administrationActions.manageDocuments);
...
xScenario('LA upload confidential and non confidential court bundle documents for a hearing', async ({I, caseViewPage, manageHearingsEventPage, manageDocumentsLAEventPage}) => {
  ...
  await I.navigateToCaseDetailsAs(config.privateSolicitorOne, caseIdAndName);
  caseViewPage.selectTab(caseViewPage.tabs.courtBundle);
