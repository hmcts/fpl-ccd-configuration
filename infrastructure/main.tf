...
data "azurerm_key_vault_secret" "general_fpla_inbox_mapping" {
  name      = "general-fpla-inbox-mapping"
  vault_uri = "${module.key-vault.key_vault_uri}"
}
...
module "case-service" {
  ...
  app_settings = {
    ...
    FPL_GENERAL_FPLA_INBOX_MAPPING                      = "${data.azurerm_key_vault_secret.general_fpla_inbox_mapping.value}"
