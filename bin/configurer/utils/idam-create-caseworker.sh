...
recreate_user() {
  echo -e "Checking IDAM user: ${email}"
  _roles=$(echo $(get_user_id_and_role "${email}") | tr -d [:space:]) # Remove whitespace
  if [[ "$_roles" == "$rolesStr" ]]; then
    echo -e "Roles equal\nNo need to delete"
    exit 0 # Nothing to delete and user already exists so just exit
  else
    echo -e "Deleting IDAM user: ${email}"
    deleteResponse=$(delete_user_request)
    if [[ "$deleteResponse" -eq 404 ]]; then
      echo -e "User ${email} doesn't exist"
      exit 1
    elif [[ "$deleteResponse" -ne 200 && "$deleteResponse" -ne 204 ]]; then
      echo "Unexpected HTTP status code from IDAM: ${deleteResponse}"
      exit 1
    else
      echo -e "Recreating IDAM user: ${email}"
      createResponse=$(create_user_request)
      if [[ "$createResponse" -eq 403 ]]; then
        echo "User ${email} already exists"
        exit 1
      elif [[ "$createResponse" -ne 201 ]]; then
        echo "Unexpected HTTP status code from IDAM: ${createResponse}"
        exit 1
      else
        echo "User ${email} - updated in IDAM"
      fi
    fi
  fi
}
